aKOB_DR_meanoriginal_fun <-
  function(data,
    ...
    signal_theta <- mapply(function(weights_pscore_W, predict_RI, theta_RI) {
      weights_pscore_W * (y - predict_RI) + (u_numeric / props["Union"]) * (predict_RI - theta_RI) + theta_RI
    },
    weights_pscore_W,
    predict_RI,
    theta_RI,
    ...
    one_boot <- function() {
      ...
      predict_RI_b <- mapply(function(Xcf, b)
        as.vector(Xcf %*% b), Xcf_b, coefs_RI_b, SIMPLIFY = FALSE)
      ...
      signal_theta_b <- mapply(function(weights_pscore_W,
                                        predict_RI,
                                        theta_RI) {
        weights_pscore_W * (y_b - predict_RI) + (u_numeric_b / props_b["Union"]) * (predict_RI - theta_RI) + theta_RI
      },
      weights_pscore_W_b,
      predict_RI_b,
      theta_RI_b,
